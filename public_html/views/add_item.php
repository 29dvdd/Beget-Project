...
function event_form_message($text, $type)
{
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (empty($_POST['csrf_token']) || !hash_equals($_SESSION['csrf_token'], $_POST['csrf_token'])) {
        $message = event_form_message('Ошибка CSRF. Обновите страницу и попробуйте снова.', 'danger');
    }
    ...
    if ($title === '' || $eventDate === '') {
        $message = event_form_message('Заполните название и дату!', 'danger');
    } elseif ($price < 0 || $tickets < 0) {
        $message = event_form_message('Цена и количество билетов не могут быть отрицательными.', 'danger');
    } else {
        if (!empty($_FILES['poster']['name'])) {
            $allowed = array('image/jpeg','image/png','image/gif');
            $max_size = 5 * 1024 * 1024;
            $finfo = finfo_open(FILEINFO_MIME_TYPE);
            $mime = $finfo ? finfo_file($finfo, $_FILES['poster']['tmp_name']) : false;
            if ($finfo) {
                finfo_close($finfo);
            }

            if (!in_array($mime, $allowed, true)) {
                $message = event_form_message('Только JPG / PNG / GIF', 'danger');
            } elseif ($_FILES['poster']['size'] > $max_size) {
                $message = event_form_message('Файл слишком большой (максимум 5MB)', 'danger');
            } else {
                ...
                if (move_uploaded_file($_FILES['poster']['tmp_name'], __DIR__ . '/../' . $name)) {
                    $imgUrl = $name;
                } else {
                    $message = event_form_message('Ошибка загрузки файла', 'danger');
                }
            }
        }
        ...
        if ($message === '') {
            $stmt = $pdo->prepare("INSERT INTO events (title, description, event_date, venue, price, available_tickets, poster_url) VALUES (?, ?, ?, ?, ?, ?, ?)");
            $stmt->execute(array($title, $desc, $eventDate, $venue ? $venue : null, $price, $tickets, $imgUrl));
            $_SESSION['flash_success'] = 'Мероприятие добавлено.';
...
<div class="container mt-4" style="max-width: 720px;">
    <div class="d-flex justify-content-between mb-3">
        ...
        <a href="index.php?page=home" class="btn btn-outline-secondary btn-sm">Назад</a>
    </div>
        ...
        <div class="card-body">
            <form method="post" enctype="multipart/form-data">
                ...
                <div class="mb-3">
                    ...
                    <input class="form-control" type="text" name="venue" placeholder="Например: Концертный зал Олимпийский">
                </div>
                <div class="row">
