...
$dateValue   = $_GET['date'] ?? '';
$sortValue   = $sortValue ?? ($_GET['sort'] ?? 'date_asc');
        ...
        <form class="row g-3 align-items-end" method="GET" action="index.php">
            ...
            <div class="col-md-3">
                ...
                <select name="sort" class="form-select">
                    <option value="date_asc" <?= $sortValue === 'date_asc' ? 'selected' : '' ?>>Ближайшие сначала</option>
                    <option value="date_desc" <?= $sortValue === 'date_desc' ? 'selected' : '' ?>>Сначала поздние</option>
                    <option value="price_asc" <?= $sortValue === 'price_asc' ? 'selected' : '' ?>>Сначала дешевле</option>
                    <option value="price_desc" <?= $sortValue === 'price_desc' ? 'selected' : '' ?>>Сначала дороже</option>
                </select>
            </div>
                    ...
                    <div class="card h-100 shadow-sm">
                        ...
                        <div class="card-body d-flex flex-column">
                            ...
                            <div class="d-flex align-items-center gap-2 mb-3">
                                ...
                                <span class="badge <?= $tickets > 0 ? 'bg-info text-dark' : 'bg-danger' ?>">
                                    <?= $tickets > 0 ? 'Есть билеты' : 'Нет билетов' ?>
                                </span>
                            </div>
                ...
                <ul class="pagination justify-content-center">
                    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                        <li class="page-item <?= ($i === $currentPage) ? 'active' : '' ?>">
                            <a class="page-link"
                               href="index.php?page=home&p=<?= $i ?>&search=<?= urlencode($searchValue) ?>&date=<?= urlencode($dateValue) ?>&sort=<?= urlencode($sortValue) ?>">
                                <?= $i ?>
                            </a>
                        </li>
                    <?php endfor; ?>
