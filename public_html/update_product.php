...
// Валидация данных
if ($id <= 0 || $title === '' || $price <= 0 || $tickets < 0 || $event_date === '') {
...
$stmt = $pdo->prepare("
    UPDATE products
    SET title = ?, description = ?, price = ?, available_tickets = ?, image_url = ?, event_date = ?, address = ?, latitude = ?, longitude = ?
    WHERE id = ?
    LIMIT 1
");
$stmt->execute([$title, $desc, $price, $tickets, $image, $event_date, $address ?: null, $latitude, $longitude, $id]);
